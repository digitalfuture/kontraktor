 ...
 * Run: node scripts/add-gapensi.js
 */
...
const existingTemplate = db.prepare('SELECT id FROM email_templates WHERE name = ? AND deleted_at IS NULL').get(templateName);
let templateId;
if (existingTemplate) {
...
const listDescription = 'Contact list from GAPENSI (Gabungan Pelaksana Konstruksi Nasional Indonesia) member directory. Includes verified construction companies across all Indonesian provinces. Compiled July 2026.';
...
const contacts = [
  ...
  { email: 'richtieglobal805@gmail.com', name: 'CV. Richtie Global', company: 'CV. Richtie Global' },
  // PT companies
...
for (const c of contacts) {
  const dup = checkDuplicate.get(listId, c.email.toLowerCase().trim());
  if (dup) {
...
console.log(`   Total in list: ${total.c}`);

db.close();
console.log('\n✅ Done! Data added to database.');
console.log('   You can now see them at: /admin/email (templates) and /admin/email/lists (mailing lists)');
